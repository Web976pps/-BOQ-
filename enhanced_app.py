    ...
    def detect_a1_format(self, pdf_path):
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                is_a1_portrait = (abs(width_mm - 594) < tolerance and abs(height_mm - 841) < tolerance)
                is_a1_landscape = (abs(width_mm - 841) < tolerance and abs(height_mm - 594) < tolerance)
    ...
    def detect_wall_contours(self, image):
        ...
        try:
            ...
            lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=100, 
                                  minLineLength=self.min_wall_length, maxLineGap=10)
    ...
    def dbscan_zone_clustering(self, text_positions, contours):
        ...
        try:
            ...
            for pos in text_positions:
                coordinates.append([pos['x'] + pos['w']/2, pos['y'] + pos['h']/2])  # Center points
    ...
    def create_zone_polygons(self, clusters, image_dimensions):
        ...
        try:
            ...
            for cluster in clusters:
                ...
                min_x = min(x_coords)
                max_x = max([x + w for x, w in zip(x_coords, w_coords)])
                min_y = min(y_coords)
                max_y = max([y + h for y, h in zip(y_coords, h_coords)])
                ...
                polygon = [
                    ...
                    (min(image_dimensions[0], max_x + padding), min(image_dimensions[1], max_y + padding)),
    ...
    def validate_completeness(self):
        ...
        for zone_id, zone_data in self.zone_registry.items():
            ...
            if zone_data['furniture_codes']:
                ...
                validation_results['issues'].append(f"Zone '{zone_data['name']}' has no furniture codes")
...
class EnhancedZoneExtractor:
    ...
    def _calculate_zone_confidence(self, zone_name):
        ...
        architectural_terms = ['HUB', 'SPACE', 'ROOM', 'AREA', 'ZONE', 'KITCHEN', 'OFFICE', 
                              'MEETING', 'CONFERENCE', 'LOBBY', 'RECEPTION', 'STORAGE']
    ...
    def process_pdf_enhanced(self, pdf_path):
        ...
        try:
            ...
            st.info(f"PDF Format: {'A1' if is_a1 else 'Other'} ({orientation}), Dimensions: {dimensions[0]:.1f}x{dimensions[1]:.1f}mm")
            ...
            for page_num in range(min(num_pages, 5)):  # Limit for performance
                ...
                try:
                    ...
                    clusters = self.geometric_analyzer.dbscan_zone_clustering(word_positions, contours)
                    zone_polygons = self.geometric_analyzer.create_zone_polygons(clusters, image.size)
...
def display_enhanced_results(results):
    ...
    with col3:
        avg_confidence = np.mean([z.get('confidence', 0) for z in results['zones'] + results['codes']])
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Zones & Codes", "Geometric Analysis", "Processing Summary", "Validation"])
...
def main():
    ...
    st.markdown("**Production-grade OCR-based extraction with geometric analysis for architectural PDFs**")
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Start Enhanced Analysis"):
            with st.spinner("🏗️ Processing with enhanced A1 pipeline (600+ DPI, geometric analysis, validation)..."):
